...
func main() {
	...
	for _, s := range cfg.Symbols {
		...
		ds := service.NewDigWsService(symbol, balance, minBalance, minAsset, int32(assetPrecision), int32(pricePrecision), fcClient, buyLevel, sellLevel, period, bySide, dbSource, depthType)
		go ds.Run()
		go ds.SaveOrder()
		go ds.RunWs()
	}
